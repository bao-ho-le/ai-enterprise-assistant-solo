...
// appendAccessScope() chỉ thao tác StringBuilder + Map (không đụng EntityManager),
// nên test thẳng JPQL/param sinh ra mà không cần Spring context hay database.
class DocumentRepositoryCustomImplTest {

    private final DocumentRepositoryCustomImpl repository = new DocumentRepositoryCustomImpl();
    ...
    void adminOrSupervisorUnrestrictedScopeSkipsFiltering() {
        ...
        assertEquals("SELECT d FROM Document d WHERE 1=1", jpql.toString());
    ...
    void employeeOrManagerRestrictedScopeAppliesAbacFilter() {
        ...
        DocumentAccessScope restricted = new DocumentAccessScope(
                false, 10L, 2L, List.of(101L, 102L));
        ...
        assertEquals(List.of(101L, 102L), params.get("scopeSharedDocumentIds"));
    }

    @Test
    void restrictedScopeWithoutDepartmentOrSharesOnlyFiltersByOwner() {

        DocumentAccessScope restricted = new DocumentAccessScope(false, 10L, null, List.of());
        ...
        String sql = jpql.toString();
        assertTrue(sql.contains("d.owner.id = :scopeUserId"));
        assertFalse(sql.contains("scopeDepartmentId"));
        assertFalse(sql.contains("scopeSharedDocumentIds"));
    }
}
